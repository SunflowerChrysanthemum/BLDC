...
void pwm_Init(void)
{
	...
	TIM_OCInitStructure.TIM_Pulse = PWM_PERIOD * pwm_duty / 100;
...
void BLDC_Control(int32_t dir_t,float duty)
{
	dir = dir_t;            /* 方向 */
    pwm_duty = duty;      /* 占空比 */
}
...
void BLDC_Change(uint8_t step_now)
{
	switch(step_now)
	{
		case 6: //b+  c-
		{
			...
			TIM_SetCompare2(TIM1, pwm_duty * PWM_PERIOD / 100);
			GPIO_SetBits(GPIOB, GPIO_Pin_14);
		...
		case 3: //c+ a-
		{
			...
			TIM_SetCompare3(TIM1, pwm_duty * PWM_PERIOD / 100);
			GPIO_SetBits(GPIOB, GPIO_Pin_15);
		...
		case 5: //a+ b-
		{
			TIM_SetCompare1(TIM1, pwm_duty * PWM_PERIOD / 100);
			GPIO_SetBits(GPIOB, GPIO_Pin_13);
