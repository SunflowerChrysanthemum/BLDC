...
enum
{
    ...
    CTRL_MODE_RC,
    CTRL_MODE_CAN,
    CTRL_MODE_Rotate_Hander
};

#define CTRL_MODE_SET 	CTRL_MODE_Rotate_Hander
...
int main(void)
{
	...
	while(1)
	{
		...
		{
			case CTRL_MODE_Rotate_Hander:
			/* 转把范围为1200~3000，对应pwm范围为0~800 */
				Rotate_adc = ADC_Buf[0];
				if (Rotate_adc < THROTTLE_MIN) Rotate_adc = THROTTLE_MIN;
				if (Rotate_adc > THROTTLE_MAX) Rotate_adc = THROTTLE_MAX;
				pwm_receive = (Rotate_adc - THROTTLE_MIN) * 4 / 9;
				if (pwm_receive > PWM_RANGE) pwm_receive = PWM_RANGE;
