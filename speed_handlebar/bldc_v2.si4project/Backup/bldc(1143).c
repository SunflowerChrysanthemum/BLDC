...
uint8_t hall_get_position(void)
{
	uint8_t h;
	h = 0;
	if(GPIO_ReadInputDataBit(HALL_PORT, HALL1_PIN) == Bit_SET) 	//H1
	{
		h |= 0x04;
	}
	if(GPIO_ReadInputDataBit(HALL_PORT, HALL2_PIN) == Bit_SET)	//H2
	{
		h |= 0x02;
	}
	if(GPIO_ReadInputDataBit(HALL_PORT, HALL3_PIN) == Bit_SET)	//H3
	{
		h |= 0x01;
	}
